...
int (*myEvdevSelect)(int nfds, fd_set *restrict readfds, fd_set *restrict writefds, fd_set *restrict exceptfds,
...
void *controllerThread(void *_args)
{
    ...
    while (args->controllers->threadsRunning)
    {
        ...
        if (myEvdevSelect(fd + 1, &file_descriptor, NULL, NULL, &tv) < 1)
            continue;
        ...
        if (read(fd, &event, sizeof(event)) == sizeof(event))
        {
            switch (event.type)
            {
                ...
                case EV_KEY:
                {
                    args->controller->keyStates[event.code] = event.value;

                    int exitTriggered = 0;
                    for (int c = 0; c < args->controller->numExitCombos; c++)
                    {
                        int allPressed = 1;
                        for (int k = 0; k < args->controller->exitCombos[c].numKeys; k++)
                        {
                            int code = args->controller->exitCombos[c].evCodes[k];
                            if (args->controller->keyStates[code] == 0)
                            {
                                allPressed = 0;
                                break;
                            }
                        }
                        if (allPressed && args->controller->exitCombos[c].numKeys > 0)
                        {
                            exitTriggered = 1;
                            break;
                        }
...
ControllerStatus startControllerThreads(Controllers *controllers)
{
    for (int i = 0; i < controllers->count; i++)
    {
        ...
        {
            controllers->controller[i].keyStates[j] = 0;
        ...
        char configCopy[INPUT_STRING_LENGTH];
        strncpy(configCopy, getConfig()->arcadeInputs.exit_game, INPUT_STRING_LENGTH);

        char *orSavePtr;
        char *orToken = strtok_r(configCopy, ",", &orSavePtr);

        while (orToken != NULL && controllers->controller[i].numExitCombos < MAX_EXIT_COMBOS)
        {
            ExitGameCombo currentCombo;
            currentCombo.numKeys = 0;

            char andCopy[INPUT_STRING_LENGTH];
            strncpy(andCopy, orToken, INPUT_STRING_LENGTH);

            char *andSavePtr;
            char *andToken = strtok_r(andCopy, "+", &andSavePtr);

            while (andToken != NULL && currentCombo.numKeys < MAX_KEYS_PER_COMBO)
            {
                while (*andToken == ' ')
                    andToken++;
                ...
                int code = -1;
                for (int m = 0; m < controllers->controller[i].inputCount; m++)
                {
                    if (strcmp(controllers->controller[i].inputs[m].inputName, andToken) == 0 ||
                        strcmp(controllers->controller[i].inputs[m].inputTechName, andToken) == 0 ||
                        strcmp(controllers->controller[i].inputs[m].inputTechNegName, andToken) == 0)
                    {
                        code = controllers->controller[i].inputs[m].evCode;
                        break;
                    }
                }
                ...
                if (code != -1)
                {
                    currentCombo.evCodes[currentCombo.numKeys++] = code;
                }
                andToken = strtok_r(NULL, "+", &andSavePtr);
            }

            if (currentCombo.numKeys > 0)
            {
                controllers->controller[i].exitCombos[controllers->controller[i].numExitCombos++] = currentCombo;
            }
...
ControllerStatus initEvdevControllers(Controllers *controllers)
{
    ...
    myEvdevSelect = dlsym(RTLD_NEXT, "select");
